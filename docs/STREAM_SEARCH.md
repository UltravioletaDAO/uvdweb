...
3. Otherwise it rebuilds the whole index (~20 s), refuses to publish if more than 2
   streams went missing (drive not mounted, half-deleted corpus), uploads it to S3,
   forces a Lambda cold start (`update_function_configuration` with the description
   `index <built_at> auto (<N> streams)`) and checks that `GET /stats` shows the new
   `built_at`.
...
.\install_stream_search_task.ps1            # hourly, hidden (pythonw), first run in 2 minutes
...
new builder republishes on the next run. The task needs the machine's AWS
credentials to allow `s3:PutObject` on `ultravioletadao/stream-search/*` and
`lambda:UpdateFunctionConfiguration` + `GetFunctionConfiguration` on
`uvd-stream-search`.

## Manual refresh (backup)

On the streamer's machine, the same as the task but now:
`python refresh_stream_search.py --force` (or `--dry-run` to build and check
without publishing). Without the task, from WSL:
