...
(async () => {
  ...
  const card = {
    ...
    x402: {
      ...
      description:
        'x402 gasless stablecoin payments through the UltravioletaDAO facilitator. Verify and settle are HTTP endpoints of the facilitator (the URLs above) and also tools of its own MCP server, x402_verify and x402_settle, declared with the rest of its tools in its server card https://facilitator.ultravioletadao.xyz/.well-known/mcp/server-card.json. This MCP server does not proxy them: its get_facilitator_networks tool reads the live network list from /supported, and it never charges for anything.',
    },
  };
